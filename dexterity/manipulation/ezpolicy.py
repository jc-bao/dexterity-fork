...
def ezpolicy(obs):
  if isinstance(obs, np.ndarray):
    dims = [4,4,3,7]
    keys = ['goal_state', 'prop/orientation', 'prop/position', 'roller_hand/joint_positions']
    list_flattened = np.split(obs, np.cumsum(dims[:-1]), axis=-1)
    obs = OrderedDict(list(zip(keys, list_flattened)))
  # ===parse observation===
  goal_orn = R.from_quat(np.concatenate([obs['goal_state'][...,1:], obs['goal_state'][...,[0]]], axis=-1))
  obj_orn = R.from_quat(
